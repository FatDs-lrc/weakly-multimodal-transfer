...
--niter=30 --niter_decay=70 --verbose --kd_fix_iter=10 --kd_loss_type=$kd_loss_type
--batch_size=256 --lr=5e-4 --dropout_rate=0.5 --run_idx=$run_idx
--name=A_ts_break5 --suffix=Adnn{mid_layers_a},{mid_layers_fusion}_loss{kd_loss_type}_kd{kd_weight}_temp{kd_temp}_ce{ce_weight}_mmd{mmd_weight}_run{run_idx}
--cvNo=$i"

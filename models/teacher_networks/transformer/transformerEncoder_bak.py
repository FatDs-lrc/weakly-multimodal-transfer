...
from transformer.position_embedding import SinusoidalPositionalEmbedding
from transformer.multihead_attention import MultiheadAttention
# from transformer.decoder_module import multihead_attention
import math
...
class TransformerEncoderLayer(nn.Module):
    ...
    *args.encoder_normalize_before* to ``True``.
    ...
    def __init__(self, embed_dim, num_heads=4, attn_dropout=0.1, relu_dropout=0.1, res_dropout=0.1,
        ...
        # self.self_attn = multihead_attention(
        #     num_units=self.embed_dim, 
    ...
    def forward(self, x, x_k=None, x_v=None):
        ...
        # mask = buffered_future_mask(x, x_k) if False else None

        if x_k is None and x_v is None:
            x, _ = self.self_attn(query=x, key=x, value=x, attn_mask=mask)
            #   x = self.self_attn(queries=x, keys=x, values=x)
        else:
            x_k = self.maybe_layer_norm(0, x_k, before=True)
            x_v = self.maybe_layer_norm(0, x_v, before=True) 
            x, _ = self.self_attn(query=x, key=x_k, value=x_v, attn_mask=mask)
            # x = self.self_attn(queries=x, keys=x_k, values=x_v)
